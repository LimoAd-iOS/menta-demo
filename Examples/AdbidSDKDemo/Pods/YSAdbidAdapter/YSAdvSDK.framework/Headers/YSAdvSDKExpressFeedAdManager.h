...
//    4. 在 expressFeedAdManagerSuccessToLoad:views: 回调中拿到 YSAdvSDKExpressFeedAdView 数组
...
@protocol YSAdvSDKExpressFeedAdManagerDelegate <NSObject>

@optional

/// 模版信息流广告加载成功，返回渲染好的 view 数组
/// @param manager 管理器实例
/// @param views 模版广告 view 数组，每个元素为 YSAdvSDKExpressFeedAdView
- (void)expressFeedAdManagerSuccessToLoad:(YSAdvSDKExpressFeedAdManager *)manager
                                    views:(NSArray<YSAdvSDKExpressFeedAdView *> *)views;

/// 模版信息流广告加载失败
/// @param manager 管理器实例
/// @param error 错误信息
- (void)expressFeedAdManager:(YSAdvSDKExpressFeedAdManager *)manager
            didFailWithError:(NSError *)error;

@end


@interface YSAdvSDKExpressFeedAdManager : NSObject

/// 委托对象
@property (nonatomic, weak) id<YSAdvSDKExpressFeedAdManagerDelegate> delegate;
